...
const [model = "gpt-5.4", repeatsArg = "1", iterationsArg = "1", suiteId = "core-designs"] =
...
const summary = await runBenchmark({
  ...
  onResult: r => {
    ...
    console.log(
      `${r.caseId.padEnd(22)} #${r.repeat} ${status} score=${score} err=${String(r.errorCount ?? "-").padStart(2)} repairs=${String(r.repairCount ?? "-").padStart(2)} iters=${r.iterationsUsed} path=[${r.scorePath.join("->")}] ${(r.durationMs / 1000).toFixed(0)}s${exp}${r.errorMessage ? " " + r.errorMessage.slice(0, 70) : ""}`
    );
  },
});
...
console.log(`mean iters    ${s.meanIterations?.toFixed(1) ?? "n/a"}`);

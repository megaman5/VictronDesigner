...
For the general request: sign in, then open "Custom Components" in the left library panel and hit "Build a Component". That opens an editor where you set a name, subtitle and size, then click near the edge of the body to drop a terminal wherever you need one \u2014 drag it to reposition, and set its id, label, type (positive/negative/AC/ground/etc.) and which side it exits from. You can also declare which DC voltages (12/24/48V) the part runs at, so the design checks know what to compare against. Save it and it shows up in that same section, ready to drag onto the canvas like any built-in part. You can come back and edit or delete a definition later; parts you've already placed keep working off their own snapshot even if you change the definition afterward, so nothing already wired breaks underfoot.

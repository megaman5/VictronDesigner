  ...
  const renderShape = () => {
    switch (type) {
      ...
      case "busbar-negative": {
        const isPositive = type === "busbar-positive";
        const barColor = isPositive ? "hsl(var(--wire-positive))" : "hsl(var(--wire-negative))";
        const barStroke = isPositive ? "#7f1d1d" : "#000000";
        const screws = [40, 60, 80, 100, 120, 140];
        const lynx = !!properties.lynxStyle;

        if (lynx) {
          // Victron Lynx-style distributor: blue module with fused slots.
          return (
            <svg width="200" height="60" viewBox="0 0 200 60">
              <rect x="10" y="12" width="180" height="38" rx="6" fill="hsl(var(--victron-blue))" stroke="hsl(var(--victron-blue-light))" strokeWidth="2" />
              <text x="100" y="23" textAnchor="middle" className="fill-white text-[9px] font-bold">LYNX {isPositive ? "+" : "−"}</text>
              {screws.map((x, i) => (
                <g key={i}>
                  {/* fuse slot */}
                  <rect x={x - 6} y="28" width="12" height="16" rx="2" fill="#0a1a2e" stroke="hsl(var(--victron-blue-light))" strokeWidth="0.5" />
                  {/* connection point at terminal center (y=30) */}
                  <circle cx={x} cy="30" r="3.5" fill={barColor} stroke="#ffffff" strokeWidth="1" />
                </g>
              ))}
            </svg>
          );
        ...
        // Thin colored bar with connection points at the vertical center, so
        // wires can exit cleanly upward or downward.
        return (
          <svg width="200" height="60" viewBox="0 0 200 60">
            <rect x="10" y="24" width="180" height="12" rx="6" fill={barColor} stroke={barStroke} strokeWidth="1.5" />
            {/* small polarity marker */}
            <text x="20" y="34" textAnchor="middle" className="fill-white text-[10px] font-bold">{isPositive ? "+" : "−"}</text>
            {screws.map((x, i) => (
              <circle key={i} cx={x} cy="30" r="3.5" fill="#ffffff" stroke={barStroke} strokeWidth="1" />
            ))}
          </svg>
        );
      }
